...
/// The "test" that starts the Lasso driver HTTP server.
...
final class DriverTest: XCTestCase {
    ...
    func testStartDriver() throws {
        ...
        let bundleId = ProcessInfo.processInfo.environment["LASSO_TARGET_BUNDLE_ID"]

        let handler = RequestHandler(bundleId: bundleId)

        // Read port from environment, default to 22088
        let port = UInt16(ProcessInfo.processInfo.environment["LASSO_DRIVER_PORT"] ?? "22088") ?? 22088

        let server = try DriverServer(handler: handler, port: port)
        try server.start()

        print("[LassoDriver] Driver ready. Target: \(bundleId ?? "default app")")
        print("[LassoDriver] Endpoints: /health, /hierarchy, /tap, /swipe, /type, /source")
