...
/// Runs in the XCUITest process, handles requests from the Grantiva CLI.
final class DriverServer: @unchecked Sendable {
    ...
    func start() throws {
        ...
        listener?.stateUpdateHandler = { state in
            switch state {
            case .ready:
                print("[GrantivaDriver] Server listening on port \(serverPort)")
            case .failed(let error):
                print("[GrantivaDriver] Server failed: \(error)")
            default:
                break
            }
        }
