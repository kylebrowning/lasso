...
public struct ScreenNavigator: Sendable {
    public var captureAll: @Sendable ([GrantivaConfig.Screen], UIAutomation, String) async throws -> [ScreenCapture]

    public init(
        captureAll: @escaping @Sendable ([GrantivaConfig.Screen], UIAutomation, String) async throws -> [ScreenCapture]
    ) {
...
extension ScreenNavigator {
    ...
    static func executeSteps(
        _ steps: [GrantivaConfig.Screen.Step],
        ui: UIAutomation
...
extension ScreenNavigator {
    public static let failing = ScreenNavigator { _, _, _ in
        throw GrantivaError.commandFailed("ScreenNavigator.failing", 1)
    }
}
