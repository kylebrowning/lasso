        ...
        self.init(
            start: { config in
                ...
                guard FileManager.default.fileExists(atPath: cachedPath) else {
                    throw GrantivaError.driverCacheStale
                }

                let cachedDir = URL(fileURLWithPath: cachedPath).deletingLastPathComponent().path
                let tempXctestrun = "\(cachedDir)/GrantivaDriver_run.xctestrun"
                ...
                var plutilCmds = [
                    "plutil -replace '\(envKeyPath).GRANTIVA_DRIVER_PORT' -string '\(config.port)' '\(tempXctestrun)'"
                ]
                if let bundleId = config.targetBundleId {
                    plutilCmds.append(
                        "plutil -replace '\(envKeyPath).GRANTIVA_TARGET_BUNDLE_ID' -string '\(bundleId)' '\(tempXctestrun)'"
                    )
                }
                ...
                process.arguments = [
                    ...
                    "-only-testing", "GrantivaDriverUITests/DriverTest/testStartDriver",
                ]
                ...
                let stderrPath = FileManager.default.temporaryDirectory
                    .appendingPathComponent("grantiva_driver_stderr_\(ProcessInfo.processInfo.processIdentifier).log")
...
extension DriverManager {
    public static let failing = DriverManager(
        start: { _ in throw GrantivaError.commandFailed("DriverManager.failing: start", 1) },
        stop: { throw GrantivaError.commandFailed("DriverManager.failing: stop", 1) },
        isRunning: { false }
    )
}
