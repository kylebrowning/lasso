...
private func driverGet(baseURL: String, path: String) async throws -> Data {
    guard let url = URL(string: "\(baseURL)\(path)") else {
        throw GrantivaError.invalidArgument("Invalid URL: \(baseURL)\(path)")
    }
    let data: Data
    ...
    do {
        ...
        throw GrantivaError.driverNotRunning
    }
...
extension DriverClient {
    public static let failing = DriverClient(
        health: { throw GrantivaError.commandFailed("DriverClient.failing: health", 1) },
        hierarchy: { throw GrantivaError.commandFailed("DriverClient.failing: hierarchy", 1) },
        tap: { _ in throw GrantivaError.commandFailed("DriverClient.failing: tap", 1) },
        swipe: { _ in throw GrantivaError.commandFailed("DriverClient.failing: swipe", 1) },
        typeText: { _ in throw GrantivaError.commandFailed("DriverClient.failing: typeText", 1) },
        pageSource: { throw GrantivaError.commandFailed("DriverClient.failing: pageSource", 1) },
        screenshot: { throw GrantivaError.commandFailed("DriverClient.failing: screenshot", 1) }
    )
}
