            ...
            do {
                ...
                case .assertVisible(let label):
                    ...
                    guard tree.find(label: label) != nil else {
                        throw GrantivaError.invalidArgument("Assertion failed: \"\(label)\" is not visible")
                    }

                case .assertNotVisible(let label):
                    let tree = try await ui.hierarchy()
                    if tree.find(label: label) != nil {
                        throw GrantivaError.invalidArgument("Assertion failed: \"\(label)\" is visible but should not be")
                    }
...
extension GrantivaConfig.Screen.Step {
