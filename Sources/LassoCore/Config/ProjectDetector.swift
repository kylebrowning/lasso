...
extension ProjectDetector {
    public static let live = ProjectDetector {
        ...
        guard let contents = try? fm.contentsOfDirectory(atPath: cwd) else {
            throw LassoError.invalidArgument("Cannot read current directory")
        }
        ...
        if let workspace {
            ...
            throw LassoError.invalidArgument("No .xcworkspace or .xcodeproj found in current directory")
        }

        let listOutput = try await shell(listCmd)
        guard let listData = listOutput.data(using: .utf8) else {
            throw LassoError.invalidArgument("Could not parse xcodebuild -list output")
        }
        ...
        guard let scheme = schemes.first else {
            throw LassoError.invalidArgument("No schemes found in Xcode project")
        }
...
extension ProjectDetector {
    private static let cachePath = ".lasso/config.json"
    ...
    public static func saveCache(_ project: DetectedProject) {
        let fm = FileManager.default
        let dir = URL(fileURLWithPath: ".lasso")
...
extension ProjectDetector {
    public static let failing = ProjectDetector {
        throw LassoError.invalidArgument("ProjectDetector.failing")
    }
}
