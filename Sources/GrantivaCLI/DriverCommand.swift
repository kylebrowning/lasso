...
struct DriverBuildCommand: AsyncParsableCommand {
    ...
    func run() async throws {
        ...
        if options.json {
            ...
            print("Driver cached at ~/.grantiva/driver/")
...
struct DriverStopCommand: AsyncParsableCommand {
    ...
    func run() async throws {
        // Find and kill any running xcodebuild test-without-building for GrantivaDriverUITests
        let output = try await shell("pgrep -f 'xcodebuild.*GrantivaDriverUITests' || true")
